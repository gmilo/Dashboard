...
type UpstreamResponse = {
  ...
  meta?: {
    tags?: Record<string, number> | Array<unknown>;
    categories?: Record<string, number> | Array<unknown>;
  };
  data?: DrinksRow[];
  error?: string;
};
...
function buildMeta(rows: DrinksRow[]) {
...
export async function GET(request: Request) {
  ...
  const upstreamUrl = new URL(`${config.dataBaseUrl}/drinks.php`);
  ...
  // Optional backend filter (may be ignored upstream until implemented there)
