...
type ToppingsResponse = {
  success: boolean;
  assignedCompanyCount?: number;
  companies?: Array<{ id: number; name: string }>;
  totals?: { total_qty: number; total_price: number };
  data?: ToppingRow[];
  error?: string;
};
...
export function ToppingsReport({ todayISO }: { todayISO: string }) {
  ...
  const key = ["toppings", range.from, range.to, companyId] as const;
  const { data, error, isLoading } = useSWR<ToppingsResponse>(key, async () => {
    const url = new URL("/api/toppings", window.location.origin);
    url.searchParams.set("date_from", range.from);
    url.searchParams.set("date_to", range.to);
    url.searchParams.set("limit", "0");
    if (companyId) url.searchParams.set("company_id", companyId);
    const res = await fetch(url.toString());
    const json = (await res.json()) as ToppingsResponse;
  ...
  if (error) {
    return (
      <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900 dark:border-rose-900/40 dark:bg-rose-950/30 dark:text-rose-100">
        Failed to load toppings: {error.message}
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="mt-3 text-xs text-slate-500 dark:text-slate-400">
          ...
          {data?.totals ? ` • Totals: ${Math.round(data.totals.total_qty)} qty, ${money(data.totals.total_price)}` : ""}
        </div>
      </section>
      ...
      {!assignedCompanyCount ? (
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 text-sm shadow-sm dark:border-slate-800 dark:bg-slate-900">
          No toppings found for this range.
        </div>
      )}
    </div>
  );
