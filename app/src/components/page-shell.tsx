...
export function PageShell({ title, headerRight, children }: { title: string; headerRight?: ReactNode; children: ReactNode }) {
  return (
    <div className="mx-auto flex min-h-dvh max-w-md flex-col bg-white pb-[calc(5.5rem+env(safe-area-inset-bottom))] dark:bg-slate-950">
      <PushAutoSync />
      ...
      <main className="flex-1 bg-slate-100 px-4 py-4 dark:bg-slate-950">{children}</main>
      <BottomNav />
    </div>
  );
}
